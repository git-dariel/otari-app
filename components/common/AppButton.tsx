...
export function AppButton({
  ...
  return (
    <Pressable
      ...
      className={`flex-row items-center justify-center rounded-full ${SIZE_CLASSNAME[size]} ${VARIANT_CLASSNAME[variant]}`}
      onPress={onPress}>
      {icon ? <MaterialIcons name={icon} size={ICON_SIZE[size]} color={iconColor} /> : null}
      <Text className={`font-bold ${TEXT_SIZE_CLASSNAME[size]} ${textColorClassName}`}>{label}</Text>
    </Pressable>
  );
}
